...
class ModelImagenet(object):
  ...
  def _encoder(self, x_input, y_in, is_train, mask_effective_attack=False):
    ...
    with tf.variable_scope('main_encoder', reuse=tf.AUTO_REUSE):
        ...
        self.filters = [32, 128, 256, 512]
        filters = self.filters

        with tf.variable_scope('unit_1_0'):
            x = self.conv_residual(x0, 64, 64, 256, self._stride_arr(1), False)

        for i in range(1, 2):
            with tf.variable_scope('unit_1_%d' % i):
                x = self.id_residual(x, 256, 64, 256, self._stride_arr(1), False)
        x1 = x
        with tf.variable_scope('unit_2_0'):
            x = self.conv_residual(x1, 256, 128, 512, self._stride_arr(2), False)  #16

        for i in range(1, 3):
          with tf.variable_scope('unit_2_%d' % i):
              x = self.id_residual(x, 512, 128, 512, self._stride_arr(1), False)

        x2 = x
        with tf.variable_scope('unit_3_0'):
            x = self.conv_residual(x2, 512, 256, 1024, self._stride_arr(2), False)  # 8
        for i in range(1, 5):
          with tf.variable_scope('unit_3_%d' % i):
              x = self.id_residual(x, 1024, 256, 1024, self._stride_arr(1), False)

        x3 = x
        with tf.variable_scope('unit_4_0'):
            x = self.conv_residual(x3, 1024, 512, 2048, self._stride_arr(2), False)  # 4
        for i in range(1, 2):
          with tf.variable_scope('unit_4_%d' % i):
              x = self.id_residual(x, 2048, 512, 2048, self._stride_arr(1), False)
